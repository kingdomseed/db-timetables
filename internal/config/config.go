...
type Config struct {
	...
	DbTimetablesClientId string          `toml:"timetables_client_id"`
	DbTimetablesApiKey   string          `toml:"timetables_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides. Deutsche Bahn Marketplace requires both
	// DB-Client-Id and DB-Api-Key on every request.
	if name, v := firstSetEnv("DB_TIMETABLES_CLIENT_ID", "DB_CLIENT_ID"); v != "" {
		cfg.DbTimetablesClientId = v
		cfg.markEnvOverride("DbTimetablesClientId")
	...
	if _, v := firstSetEnv("DB_TIMETABLES_API_KEY", "DB_API_KEY"); v != "" {
		cfg.DbTimetablesApiKey = v
		cfg.markEnvOverride("DbTimetablesApiKey")
	}
	...
	if v := os.Getenv("DB_TIMETABLES_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("DB_TIMETABLES_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "db-timetables-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.DbTimetablesClientId
	if token == "" {
		return ""
	}
	if c.DbTimetablesClientId == "" {
		return ""
	}
	return token
}
...
func (c *Config) CredentialConfigured() bool {
	...
	return c.AuthHeader() != "" && strings.TrimSpace(c.DbTimetablesApiKey) != ""
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.DbTimetablesClientId = ""
	c.DbTimetablesApiKey = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		DbTimetablesClientId: c.DbTimetablesClientId,
		DbTimetablesApiKey:   c.DbTimetablesApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.DbTimetablesClientId == "" {
		c.DbTimetablesClientId = creds.DbTimetablesClientId
	}
	if c.DbTimetablesApiKey == "" {
		c.DbTimetablesApiKey = creds.DbTimetablesApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.DbTimetablesClientId = token
	delete(c.envOverrides, "DbTimetablesClientId")
	c.updateFileConfigField("DbTimetablesClientId")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("DbTimetablesClientId")
	c.DbTimetablesApiKey = ""
	delete(c.envOverrides, "DbTimetablesApiKey")
	c.updateFileConfigField("DbTimetablesApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DbTimetablesClientId"] {
			out.DbTimetablesClientId = c.fileConfig.DbTimetablesClientId
		}
		if c.envOverrides["DbTimetablesApiKey"] {
			out.DbTimetablesApiKey = c.fileConfig.DbTimetablesApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DbTimetablesClientId":
		c.fileConfig.DbTimetablesClientId = c.DbTimetablesClientId
	case "DbTimetablesApiKey":
		c.fileConfig.DbTimetablesApiKey = c.DbTimetablesApiKey
	}
}
