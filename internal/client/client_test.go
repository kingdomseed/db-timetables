...
func TestDoSendsBothMarketplaceHeaders(t *testing.T) {
	var gotClientID, gotAPIKey string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotClientID = r.Header.Get("DB-Client-ID")
		gotAPIKey = r.Header.Get("DB-Api-Key")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"ok":true}`))
	}))
	t.Cleanup(server.Close)
	...
	c := New(&config.Config{
		BaseURL:              server.URL,
		DbTimetablesClientId: "client-id-value",
		ClientSecret:         "api-key-value",
	}, time.Second, 0)
	c.HTTPClient = server.Client()
	c.NoCache = true

	if _, err := c.Get(context.Background(), "/station/BLS", nil); err != nil {
		t.Fatalf("Get returned error: %v", err)
	}
	if gotClientID != "client-id-value" {
		t.Fatalf("DB-Client-ID = %q, want client-id-value", gotClientID)
	}
	if gotAPIKey != "api-key-value" {
		t.Fatalf("DB-Api-Key = %q, want api-key-value", gotAPIKey)
	}
}
