	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("DB-Client-ID", h)
			}
			if c.Config != nil && c.Config.DbTimetablesApiKey != "" {
				req.Header.Set("DB-Api-Key", c.Config.DbTimetablesApiKey)
			}
		} else {
			...
			req.Header.Del("DB-Client-ID")
			req.Header.Del("DB-Api-Key")
		}
		return nil
	}
	return c
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("DB-Client-ID", authHeader)
		}
		...
		if c.Config != nil {
			if v := c.Config.DbTimetablesApiKey; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export DB_TIMETABLES_API_KEY=<your-token>")
				}
				req.Header.Set("DB-Api-Key", v)
			}
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			if ua := os.Getenv("DB_TIMETABLES_USER_AGENT"); ua != "" {
				req.Header.Set("User-Agent", ua)
			} else {
				req.Header.Set("User-Agent", "db-timetables-pp-cli/1.0.274")
			}
		}
		...
		if resp.StatusCode < 400 {
			...
			// XML endpoints carry an application/xml Accept override. Use that
			// per-request signal so mixed XML/JSON specs normalize only the XML
			// endpoint's success body. A decode failure returns the raw bytes.
			if isXMLRequest(req.Header.Get("Accept")) && isXMLResponseContentType(resp.Header.Get("Content-Type")) {
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "DB-Client-ID", maskToken(authHeader))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export DB_TIMETABLES_CLIENT_ID=<your-token> or db-timetables-pp-cli auth set-token <token>")
}
...
func isXMLRequest(accept string) bool {
	for value := range strings.SplitSeq(accept, ",") {
		mt := strings.ToLower(strings.TrimSpace(strings.SplitN(value, ";", 2)[0]))
		if mt == "application/xml" || mt == "text/xml" || (strings.HasSuffix(mt, "+xml") && mt != "application/xhtml+xml") {
			return true
		}
	}
	return false
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.DbTimetablesClientId)
		addCredential(c.Config.DbTimetablesApiKey)
	}
