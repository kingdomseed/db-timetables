...
func resolveReadWithStrategyResponsePathAndJSONGuard(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, responsePath string, guardLiveJSON bool, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'db-timetables-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("db-timetables-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
		...
		if json.Unmarshal(data, &envelope) == nil {
			...
			if items == nil && !matchedListEnvelope && len(envelope) > 0 {
				...
				if !looksLikeListEnvelope {
					_, _, _ = db.UpsertTimetableResource(resourceType, []json.RawMessage{data})
					return
				}
			}
		}
	...
	if len(items) > 0 {
		_, _, _ = db.UpsertTimetableResource(resourceType, items)
	}
}
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "db-timetables-pp-cli")
	if err != nil {
	...
	applyMaxAgeFreshness(flags, hintWriter, resourceType, &prov)
...
func applyMaxAgeFreshness(flags *rootFlags, hintWriter io.Writer, resourceType string, prov *DataProvenance) {
	if flags == nil || prov == nil || flags.maxAge <= 0 || prov.SyncedAt == nil {
		return
	}
	age := time.Since(*prov.SyncedAt)
	stale := age > flags.maxAge
	flags.freshnessMeta = map[string]any{
		"max_age": flags.maxAge.String(),
		"age":     age.String(),
		"stale":   stale,
	}
	prov.Freshness = flags.freshnessMeta
	if stale && hintWriter != nil {
		fmt.Fprintf(hintWriter, "warning: local %s data is older than --max-age %s; refresh with a live command\n", resourceType, flags.maxAge)
	}
}
