...
type rootFlags struct {
	...
	// invocation. Mirrors the DB_TIMETABLES_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "db-timetables-pp-cli",
		Short: `Db Timetables CLI — Official station boards: delay, platform, cancel — not a journey planner.`,
		Long: `Db Timetables CLI — Official station boards: delay, platform, cancel — not a journey planner.
...
Health check: run 'db-timetables-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("db-timetables-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select eva,m,s)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'db-timetables-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFchgPromotedCmd(flags))
	rootCmd.AddCommand(newPlanPromotedCmd(flags))
	rootCmd.AddCommand(newRchgPromotedCmd(flags))
	rootCmd.AddCommand(newStationPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBoardCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCancellationsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDelaysCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPlatformsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWatchCmd(flags))
