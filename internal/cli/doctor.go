...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  db-timetables-pp-cli doctor
  db-timetables-pp-cli doctor --json
  db-timetables-pp-cli doctor --fail-on warn
  db-timetables-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set both credentials: export DB_TIMETABLES_CLIENT_ID=\"your-client-id\" and export DB_TIMETABLES_API_KEY=\"your-api-key\""
					report["auth_docs_url"] = "https://developers.deutschebahn.com/db-api-marketplace/apis/product/timetables"
				} else {
					authConfigured = true
			...
			if os.Getenv("DB_TIMETABLES_CLIENT_ID") != "" || os.Getenv("DB_CLIENT_ID") != "" {
				if os.Getenv("DB_TIMETABLES_CLIENT_ID") != "" {
					authEnvSet = append(authEnvSet, "DB_TIMETABLES_CLIENT_ID")
				} else {
					authEnvSet = append(authEnvSet, "DB_CLIENT_ID")
				}
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DB_TIMETABLES_CLIENT_ID")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.DbTimetablesApiKey
				}
				if os.Getenv("DB_TIMETABLES_API_KEY") != "" {
					recordAdditionalAuthEnv("DB_TIMETABLES_API_KEY", configuredValue)
				} else if os.Getenv("DB_API_KEY") != "" {
					recordAdditionalAuthEnv("DB_API_KEY", configuredValue)
				} else {
					recordAdditionalAuthEnv("DB_TIMETABLES_API_KEY", configuredValue)
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "db-timetables-pp-cli", suggestion)
						} else {
