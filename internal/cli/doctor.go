...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  db-timetables-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			} else if os.Getenv("DB_CLIENT_ID") != "" {
			...
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.ClientSecret
				}
				apiKeyName := "DB_TIMETABLES_API_KEY"
				if os.Getenv("DB_TIMETABLES_API_KEY") != "" {
					apiKeyName = "DB_TIMETABLES_API_KEY"
				} else if os.Getenv("DB_API_KEY") != "" {
					apiKeyName = "DB_API_KEY"
				} else if os.Getenv("TIMETABLES_CLIENT_SECRET") != "" {
					apiKeyName = "TIMETABLES_CLIENT_SECRET"
				}
				recordAdditionalAuthEnv(apiKeyName, configuredValue)
			}
			switch {
