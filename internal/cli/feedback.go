...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DB_TIMETABLES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DB_TIMETABLES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "db-timetables-pp-cli/feedback")
...
When ` + "`DB_TIMETABLES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DB_TIMETABLES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: "  db-timetables-pp-cli feedback \"plan is static; use board for delays\"",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.HasPrefix(text, "__printing_press_") {
				return fmt.Errorf("invalid feedback argument %q", text)
			}
			truncated := false
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  db-timetables-pp-cli feedback list --json",
