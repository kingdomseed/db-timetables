...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'db-timetables-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set your API key with: export DB_TIMETABLES_CLIENT_ID=\"your-client-id\" and DB_TIMETABLES_API_KEY=\"your-api-key\""+
			"\n      Run 'db-timetables-pp-cli doctor' to check auth status.", err))
...
func printOutputWithFlagsMeta(w io.Writer, data json.RawMessage, flags *rootFlags, agentMeta map[string]any) error {
	...
	if flags.csv || flags.plain {
		data = collectionItemsForOutput(data, "")
	}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set your API key with: export DB_TIMETABLES_CLIENT_ID=\"your-client-id\" and DB_TIMETABLES_API_KEY=\"your-api-key\""))
	}
	if len(trimmed) == 0 {
