...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Timetables",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  db-timetables-pp-cli auth setup\n  db-timetables-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set both Marketplace credentials:")
			fmt.Fprintln(w, "  export DB_TIMETABLES_CLIENT_ID=\"your-client-id\"")
			fmt.Fprintln(w, "  export DB_TIMETABLES_API_KEY=\"your-api-key\"")
			fmt.Fprintln(w, "  db-timetables-pp-cli auth set-token <client-id>")
			fmt.Fprintln(w, "  db-timetables-pp-cli auth set-api-key <api-key>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  db-timetables-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			w := cmd.OutOrStdout()
			authed := cfg.CredentialConfigured()
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set both Marketplace credentials:")
				fmt.Fprintln(w, "  export DB_TIMETABLES_CLIENT_ID=\"your-client-id\"")
				fmt.Fprintln(w, "  export DB_TIMETABLES_API_KEY=\"your-api-key\"")
				fmt.Fprintf(w, "  db-timetables-pp-cli auth set-token <client-id>\n")
				fmt.Fprintf(w, "  db-timetables-pp-cli auth set-api-key <api-key>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  db-timetables-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthSetApiKeyCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Short:   "Save the DB-Api-Key (client secret) to the credentials file",
		Example: "  db-timetables-pp-cli auth set-api-key YOUR_API_KEY",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  db-timetables-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			for _, name := range []string{"DB_TIMETABLES_CLIENT_ID", "DB_CLIENT_ID", "DB_TIMETABLES_API_KEY", "DB_API_KEY", "TIMETABLES_CLIENT_SECRET"} {
				if os.Getenv(name) != "" {
					if envStillSet != "" {
						envStillSet += ", " + name
					} else {
