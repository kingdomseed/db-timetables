...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Deutsche Bahn Timetables",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://developers.deutschebahn.com/db-api-marketplace/apis/product/timetables")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export DB_TIMETABLES_CLIENT_ID=\"your-token-here\"")
			fmt.Fprintln(w, "  db-timetables-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://developers.deutschebahn.com/db-api-marketplace/apis/product/timetables"
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export DB_TIMETABLES_CLIENT_ID=\"your-token-here\" # Marketplace client id sent as DB-Client-Id. Also accepts DB_CLIENT_ID.")
				fmt.Fprintf(w, "  db-timetables-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("DB_TIMETABLES_CLIENT_ID") != "" {
				envStillSet = "DB_TIMETABLES_CLIENT_ID"
			}
