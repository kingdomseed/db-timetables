...
// db-timetables-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"db-timetables-pp-cli/internal/learn/entities"
	"db-timetables-pp-cli/internal/learn/lookups"
	"db-timetables-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "DB_TIMETABLES_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("db-timetables-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  db-timetables-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  db-timetables-pp-cli recall "<question>" --agent
  db-timetables-pp-cli recall "<question>" --agent --min-confidence 2`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  db-timetables-pp-cli learnings forget "<question>" --resource <id>
  db-timetables-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  db-timetables-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  db-timetables-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
