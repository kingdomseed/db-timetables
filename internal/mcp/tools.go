...
import (
	...
	"db-timetables-pp-cli/internal/cli"
	"db-timetables-pp-cli/internal/client"
	"db-timetables-pp-cli/internal/cliutil"
	"db-timetables-pp-cli/internal/config"
	"db-timetables-pp-cli/internal/learn"
	"db-timetables-pp-cli/internal/mcp/bound"
	"db-timetables-pp-cli/internal/mcp/cobratree"
...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		...
		makeAPIHandler("GET", "/fchg/{evaNo}", true, false, map[string]string{"Accept": "application/xml"}, mcpPageConfig{}, []mcpParamBinding{{PublicName: "evaNo", WireName: "evaNo", Location: "path"}}, []string{"evaNo"}),
	)
	s.AddTool(
		...
		makeAPIHandler("GET", "/plan/{evaNo}/{date}/{hour}", true, false, map[string]string{"Accept": "application/xml"}, mcpPageConfig{}, []mcpParamBinding{{PublicName: "evaNo", WireName: "evaNo", Location: "path"}, {PublicName: "date", WireName: "date", Location: "path"}, {PublicName: "hour", WireName: "hour", Location: "path"}}, []string{"evaNo", "date", "hour"}),
	)
	s.AddTool(
		...
		makeAPIHandler("GET", "/rchg/{evaNo}", true, false, map[string]string{"Accept": "application/xml"}, mcpPageConfig{}, []mcpParamBinding{{PublicName: "evaNo", WireName: "evaNo", Location: "path"}}, []string{"evaNo"}),
	)
	s.AddTool(
		...
		makeAPIHandler("GET", "/station/{pattern}", true, false, map[string]string{"Accept": "application/xml"}, mcpPageConfig{}, []mcpParamBinding{{PublicName: "pattern", WireName: "pattern", Location: "path"}}, []string{"pattern"}),
	)
	// SQL tool — ad-hoc analysis on synced data without API calls
	s.AddTool(
		mcplib.NewTool("sql",
			mcplib.WithDescription("Run read-only SQL against local database. Use for ad-hoc analysis, aggregations, and joins across synced resources. Requires sync first."),
			mcplib.WithString("query", mcplib.Required(), mcplib.Description("SQL query (SELECT or WITH...SELECT). Synced records live in resources(resource_type, id, data); filter by resource_type and use json_extract on data, e.g. SELECT json_extract(data,'$.name') FROM resources WHERE resource_type='fchg'.")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
		),
		handleSQL,
		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
				return mcpToolError("authentication error: " + cliutil.SanitizeErrorBody(msg) +
					...
					"\n      Set your API key with: export DB_TIMETABLES_CLIENT_ID=\"your-token-here\"" +
					"\n      See API docs: https://developers.deutschebahn.com/db-api-marketplace/apis/product/timetables" +
					"\n      Run 'db-timetables-pp-cli doctor' to check auth status."), nil
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run db-timetables-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run db-timetables-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run db-timetables-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func mcpSQLQueryError(err error) string {
	...
	if strings.Contains(strings.ToLower(msg), "no such table") {
		return fmt.Sprintf("query failed: %v. Synced records live in resources(resource_type, id, data), not one SQL table per resource. Filter by resource_type, for example resource_type='fchg', and read JSON fields with json_extract(data,'$.field').", err)
	}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		"api":         "db-timetables",
		"description": "Official station boards: delay, platform, cancel — not a journey planner.",
		"archetype":   "generic",
		"tool_count":  4,
		"paths":       paths,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion db-timetables-pp-cli binary.",
		...
		"auth": map[string]any{
			...
			"docs_url": "https://developers.deutschebahn.com/db-api-marketplace/apis/product/timetables",
		},
		"resources": []map[string]any{
			{
				"name":        "fchg",
				"description": "Manage fchg",
		...
		"command_mirror_capabilities": []map[string]string{
			{"name": "Live station board", "command": "board", "description": "See the live board for one station this hour: planned trains with delays, platform moves, and cancels overlaid.", "rationale": "Requires joining the static hourly plan with fchg changes; no single Marketplace endpoint returns a usable board.", "via": "mcp-command-mirror"},
		...
		"playbook": []map[string]string{
			...
			{"topic": "Platform-change list", "insight": "A local/overlaid filter on planned vs changed platform; the API only returns raw XML attributes."},
			{"topic": "Cancellations this hour", "insight": "Filters overlaid plan+fchg (or the local slice) for cancelled events the raw endpoints do not isolate."},
			{"topic": "Delay list", "insight": "Computes delay from planned vs changed timestamps after the overlay join."},
			{"topic": "Cheap change watch", "insight": "Only works if a local snapshot exists; official docs tell clients to poll rchg after fchg, and no competitor CLI does that against SQLite."},
		},
	}
